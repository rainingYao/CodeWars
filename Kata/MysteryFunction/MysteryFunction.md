...
Assume `num` has `n` bits. Then `mystery(num)` is the number whose binary representation is the `num`<sup>th</sup> entry in the table `T(n)`, where `T(n)` is defined recursively as follows:
